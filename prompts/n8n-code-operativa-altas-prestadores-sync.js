// Altas prestadores — Google Sheets → Postgres
...
if (items.length === 0) {
  return [{ json: { query: '-- No hay datos en altas prestadores' } }];
}
...
const filasSQL = items.map((item) => {
  ...
  return `(
    ...
    ${nullable(pick(lookup, '¿SIGUE ACTIVO?', 'SIGUE ACTIVO'))},
...
const query = `
TRUNCATE TABLE area_operativa_ref_candela_altas;
INSERT INTO area_operativa_ref_candela_altas (
  fecha_de_ingreso,
  sede,
  sigue_activo,
  reclutadora,
